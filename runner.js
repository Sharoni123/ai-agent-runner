...
function buildArticleParagraphs(task) {
  ...
  return [
    ...
    `כדי שטקסט יעבוד באמת, הוא צריך להישמע טבעי, אמין ומדויק. הקורא לא מחפש ניסוחים מלאכותיים, אלא תוכן שמסביר את הדברים בגובה העיניים, ועדיין משדר מקצועיות וביטחון. במקרה של ${briefTitle}, חשוב לנסח את התוכן באופן שמחבר בין מידע ברור לבין תחושת ערך, כך שהמסר לא רק יובן — אלא גם ירגיש רלוונטי ואמין.`,
    ...
    `גם למבנה עצמו יש חשיבות גדולה. טקסט טוב נבנה בהדרגה: הוא מתחיל בפתיחה שמושכת פנימה, ממשיך לפסקאות שמרחיבות את ההבנה, ומסתיים בצורה שמשאירה תחושה ברורה של כיוון, ערך או הזדמנות. כאשר המעבר בין החלקים זורם בצורה טבעית, הקורא נשאר עם הטקסט עד הסוף בלי להרגיש שמכבידים עליו או מנסים למכור לו בצורה אגרסיבית מדי.`,

    `הטון המבוקש כאן הוא ${tone}, ולכן השפה צריכה להיות נקייה, בטוחה ורהוטה. לא צריך להשתמש בסיסמאות מוגזמות כדי לשכנע. להפך — דווקא ניסוח מדויק, שקול וקריא עובד הרבה יותר טוב ברוב המקרים. כאשר התוכן כתוב היטב, הקורא מרגיש שיש מולו מסר מסודר, כזה שמבין את הצרכים שלו, מדבר בשפה שמתאימה לו, ונותן לו סיבה טובה להמשיך לחשוב על מה שקרא גם אחרי שסיים.`,
...
function buildArticleRevisionParagraphs(task, previousOutput, notes) {
  ...
  return [
    ...
    `בתהליך השכתוב ניתן דגש גם על חוויית הקריאה. פסקאות שאולי נשמעו כלליות מדי קיבלו ניסוח ממוקד יותר, המעברים בין החלקים שופרו, והמסר המרכזי קיבל נוכחות חזקה יותר בתוך הטקסט. זה חשוב משום שהקורא לא בוחן רק מה כתוב, אלא גם איך זה מרגיש בזמן הקריאה — האם זה ברור, האם זה נעים, והאם זה משדר ביטחון ומקצועיות.`,
    ...
    `מעבר לתיקונים הטכניים, בוצע כאן גם חיזוק של התחושה הכללית שהטקסט מייצר. טקסט טוב לא צריך רק להיות נכון; הוא צריך להישמע נכון, להרגיש נכון, ולהתאים למטרה שלשמה הוא נכתב. לכן הושם דגש על טון בטוח יותר, ניסוחים רהוטים יותר וזרימה טובה יותר בין רעיון לרעיון, כדי שהקריאה תהיה רציפה, ברורה וטבעית.`,
...
function looksLikeMetaWriting(text) {
  ...
  const banned = [
    ...
    "הטקסט צריך",
    "כשכותבים נכון",
    "כאשר מחברים את כל אלה",
  ];

  return banned.some((phrase) => value.includes(phrase.toLowerCase()));
}
...
async function generateArticleWithAI(task) {
  ...
  const systemPrompt = [
    "אתה כותב תוכן מקצועי בעברית.",
    "אתה חייב לכתוב כתבה אמיתית לפרסום, לא הסבר על איך לכתוב כתבה.",
    "אסור בהחלט לכתוב משפטים מטא כמו 'כאשר כותבים כתבה', 'השלב הבא', 'חשוב להתחיל', 'כתבה טובה צריכה', 'הטקסט צריך', 'מבנה הכתבה'.",
    "אתה מדבר רק על הנושא עצמו.",
    "העברית חייבת להיות טבעית, זורמת, משכנעת ולא רובוטית.",
    "הכתבה צריכה להיות בערך 450 מילים.",
    "חייבות להיות פסקאות אמיתיות עם שורה ריקה בין כל פסקה.",
    "אין רשימות, אין סעיפים, אין הסבר על כתיבה.",
    "החזר JSON בלבד לפי הסכמה שניתנה.",
  ].join(" ");

  const userPrompt = [
    `כתוב כתבה מקצועית בעברית בנושא הבא: ${briefTitle}`,
    `קהל יעד: ${audience}`,
    `טון: ${tone}`,
    `זווית מרכזית: ${angle}`,
    `אורך מבוקש: ${wordCount} מילים`,
    `קריאה לפעולה רצויה: ${cta}`,
    `מידע נוסף:\n${additionalContext || "אין"}`,
    `נקודות מפתח להבלטה:\n${keyPointsText}`,
    mode === "revise" ? `כותרת קודמת: ${previousTitle || "אין"}` : "",
    mode === "revise" ? `כתבה קודמת:\n${previousText || "אין"}` : "",
    mode === "revise" ? `הערות תיקון:\n${revisionNotesText}` : "",
    "תכתוב את הכתבה עצמה בלבד, על הנושא עצמו.",
    "לא לכתוב על תהליך הכתיבה.",
    "title = כותרת כתבה אמיתית.",
    "subtitle = תת כותרת קצרה וטבעית.",
    "article_text = כתבה מלאה בפסקאות.",
    "seo_titles = בדיוק 3 כותרות SEO.",
    "cta = משפט הנעה לפעולה.",
  ]
    .filter(Boolean)
    .join("\n\n");
  ...
  for (let attempt = 0; attempt < 2; attempt += 1) {
    ai = await createStructuredResponse({
      model: "gpt-4.1-mini",
      systemPrompt,
      userPrompt:
        attempt === 0
          ? userPrompt
          : `${userPrompt}\n\nאזהרה: בפעם הקודמת כתבת טקסט מטא על איך כותבים כתבה. עכשיו תכתוב רק כתבה אמיתית על הנושא עצמו.`,
      schemaName: "copywriter_article",
      schema: articleSchema(),
    });

    lastText = normalizeText(ai.article_text);

    if (!looksLikeMetaWriting(lastText)) {
      break;
    }
  }

  if (!ai || looksLikeMetaWriting(lastText)) {
    throw new Error("AI returned meta-writing instead of a real article");
  }

  const articleText = clampWordRange(lastText, 430, 500);
